...
3.- Replace all occurrences of "CDialog" with "CDialogSK" in the .h and .cpp files 
    for your dialog class. 
4.- If you plan to use a background image (bitmap) go to the dialog Properties, 
    Styles tab and make it Style=Popup, Border=None, and uncheck the "Title Bar" check box. 
5.- At the end of OnInitDialog of your dialog class, add calls to the appropriate 
...
6.- If, for example, you want to make a circular dialog, 
    create a image that has a blue circle on a green background. 
    Then, call SetBitmap with the path to the image and call SetTransparentColor, 
    passing the color of the background (green). This will remove the background 
